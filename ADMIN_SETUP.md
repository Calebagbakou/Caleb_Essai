...
Ouvre `assets/supabase-config.js` (à la racine du projet — c'est la SEULE
config Supabase de tout le site : public, boutique et admin la partagent)
et remplace :
```js
window.SUPABASE_URL = "https://TON-PROJET.supabase.co";
window.SUPABASE_ANON_KEY = "TA_CLE_ANON_PUBLIQUE";
```
par tes vraies valeurs copiées à l'étape 5. ⚠️ `SUPABASE_URL` ne doit PAS
se terminer par `/rest/v1/` — juste `https://xxxxx.supabase.co`.
...
Le schéma, l'authentification, et les écrans **Messages** et **Paramètres**
sont maintenant en place et fonctionnels. Le formulaire de contact du site
public écrit aussi réellement dans Supabase.

Restent à construire (encore marqués « bientôt » dans le menu admin) :
Projets, Compétences, Services, Médias (images/vidéos), Produits,
Commandes, Clients — ainsi que la connexion du site public et de la
boutique à Supabase (ils affichent encore du contenu statique/en dur), et
le formulaire de témoignages (encore désactivé, `onsubmit="return false;"`).
